...
> #### 7、利用 traceroute 探测主机A到主机B的路由路径

[traceroute 和 tracert](https://www.cnblogs.com/zhanghongfeng/p/7238100.html)

windowns 是 tracert 命令

Linux 是 traceroute 命令



当我们主机发现访问不了其他服务的时候，可以使用该命令来探测是否是访问路径上存在问题



`traceroute host` 用来探测当前主机到目的 host 主机的路由路径，它会将路径上经过的路由器的 ip 输出
...
但是最后一个包到达目的主机的时候，即使 TTL = 1 目的主机不会丢弃，也不会返回 ICMP 超时报文，那么 traceroute 如何知道已经到达了目的主机？
Linux 发送的 UDP 报文中指定的是一个 port > 30000 的端口，该端口主机的进程不会使用，这样在 UDP 报文到达目的主机后会找不到对应通信的进程，这样的话目的主机会返回一个 ICMP 差错报文，内容是 port unreachable（端口不可达），这样 traceroute 收到该报文后就知道到达了目的主机了，最终完成全链路的追踪
```

这里我们可以看出，**ICMP 差错报文是回应 TCP/UDP 的**，比如通信过程中主机不可达、端口不可达等都会使用 ICMP 差错报文来进行返回告知源主机
