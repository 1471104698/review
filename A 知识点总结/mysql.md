...
```java
1、ACID
A ：原子性，一个事务要么提交成功，要么失败回滚，通过 undo log 来实现
C ：一致性，从一个正确的状态转移到另一个正确的状态（需要在应用层来保证）
I ：隔离性，尽管可能存在多个事务并发执行，但是对于任何一对事务 Ti 和 Tj 来说，它们彼此之间都感知不到对方的存在，每个事务都不知道有其他事务在并发执行，不同的隔离级别对事务产生不同的影响
D ：持久性，一旦事务提交，那么表示永久生效，数据持久化到磁盘上，通过 redo log 来实现
...
ReadView 称作快照，包含以下字段：
1）low_limit_id：当前 ReadView 创建时已分配的最大事务 id + 1，即下一个将要分配的事务 id
2）up_limit_id：当前 ReadView 创建时未提交的事务中最小的事务 id
3）trx_ids：当前 ReadView 创建时未提交事务的事务 id 列表（不包括当前事务的事务 id）
...
## 9、mysql select 查询 9 大优化

```java
1、为频繁查询的列建立索引，如果是联合索引，那么注意最左匹配原则，同时按照区分度进行建立
2、避免给频繁修改的列添加索引，因为频繁修改那么意味着索引树可能需要频繁的进行分裂
4、对于定长字段，使用 char，因为 char 在获取时可以根据长度进行获取，即使数据没有达到指定长度也会使用空格补齐，而 varchar 由于是可变长的，所以需要两次查询：第一次查询数据长度，第二次根据数据长度获取真实的数据 (利用空间换时间)
5、尽量避免 select *
    1）select * 在很大情况下查询优化器不会走索引，而是走聚簇索引树的全表扫描，效率低
...
10、当查询数据量大导致的速度慢时，可以使用 limit 分页查询，当使用分页查询时，如果查询的偏移量比较大，其实是可以将上一页的参数最大值作为查询条件的，以此来减少数据量
    比如 select * from test order by a limit 1000000,10;
	我们可以根据上一页的 create_time 的最大值作为查询条件
        select * from test where a > 99999 order by a limit 0，10;
	前提是 a 需要建立索引，并且 a 是唯一索引，值是唯一的，否则直接 a > 99999 的话可能会跳过未选择的 a = 99999 的数据行
```
